...
interface AdminOrderDetailsModalProps {
  order: Order;
  onClose: () => void;
  onPrintInvoice: (order: Order, mode?: 'invoice' | 'sticker' | 'challan') => void;
      ...
      >
        ...
        <div className="bg-[#0b1329] px-4 sm:px-6 py-4 text-white flex flex-wrap items-center justify-between gap-3 shrink-0 border-b border-slate-800">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {/* 1-Click Invoice Memo */}
            <button
              id="admin-print-invoice-btn"
              type="button"
              onClick={() => onPrintInvoice(order, 'invoice')}
              title="১-ক্লিকে রিটেল ইনভয়েস ক্যাশমেমো প্রিন্ট করুন"
              className="px-3 py-2 bg-[#f85606] hover:bg-[#e04a00] text-white font-bold text-xs rounded-lg flex items-center gap-1.5 transition-colors shadow-sm cursor-pointer"
            >
              <Printer className="w-3.5 h-3.5" />
              <span>ইনভয়েস মেমো</span>
            </button>

            {/* 1-Click Courier Shipping Sticker Label */}
            <button
              id="admin-print-sticker-btn"
              type="button"
              onClick={() => onPrintInvoice(order, 'sticker')}
              title="১-ক্লিকে কুরিয়ার পার্সেল স্টিকার লেবেল প্রিন্ট করুন (Steadfast/Pathao/RedX)"
              className="px-3 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs rounded-lg flex items-center gap-1.5 transition-colors shadow-sm cursor-pointer"
            >
              <Tag className="w-3.5 h-3.5" />
              <span>কুরিয়ার স্টিকার</span>
            </button>

            {/* 1-Click Packaging Challan */}
            <button
              id="admin-print-challan-btn"
              type="button"
              onClick={() => onPrintInvoice(order, 'challan')}
              title="১-ক্লিকে ওয়্যারহাউজ প্যাকিং ডেলিভারি চালান প্রিন্ট করুন"
              className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold text-xs rounded-lg flex items-center gap-1.5 transition-colors border border-slate-700 cursor-pointer hidden md:flex"
            >
            ...
            <button
              ...
              className="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-700 font-bold text-xs rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer hidden sm:flex"
            >
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 text-slate-800">
          ...
          {/* SMS DISPATCH BANNER */}
          {smsSentSuccess && (
            <div className="p-3.5 bg-indigo-50 border border-indigo-200 text-indigo-900 rounded-xl text-xs font-bold flex items-center justify-between shadow-2xs animate-in fade-in">
              ...
              <button 
                type="button" 
                onClick={() => setSmsSentSuccess(null)}
                className="text-indigo-800 hover:text-indigo-950 font-bold cursor-pointer"
              >
                ✕
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const res = StoreService.sendOrderConfirmationSMS(order);
                      setSmsSentSuccess(`অর্ডার #${order.orderNumber} এর কনফার্মেশন SMS সফলভাবে পাঠানো হয়েছে (${order.phone})!`);
                      setTimeout(() => setSmsSentSuccess(null), 4500);
                      const refreshed = StoreService.getOrderById(order.id);
                      if (refreshed) {
                        setOrder(refreshed);
                        if (onOrderUpdated) onOrderUpdated(refreshed);
                      }
                    }}
                    className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-md text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
                    title="গ্রাহকের মোবাইলে কনফার্মেশন SMS পাঠান"
                  >
            ...
            {/* SMS Notification Live Status */}
            <div className={`rounded-xl p-3 border flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs ${
              order.smsSent 
                ? 'bg-emerald-50/70 border-emerald-200 text-emerald-900' 
                : 'bg-slate-50 border-slate-200 text-slate-600'
            }`}>
              <div className="flex items-center gap-2">
                <CheckCircle2 className={`w-4 h-4 ${order.smsSent ? 'text-emerald-600' : 'text-slate-400'} shrink-0`} />
                <div>
                  <span className="font-bold">
                    {order.smsSent ? 'SMS নোটিফিকেশন ডেলিভার্ড' : 'SMS নোটিফিকেশন পাঠানো হয়নি'}
                  </span>
                  {order.smsSentAt && (
                    <span className="text-[11px] text-slate-500 ml-1.5 font-normal">
                      ({new Date(order.smsSentAt).toLocaleString('en-GB')})
                    </span>
                  )}
                  {order.smsContent && (
                    <p className="text-[11px] text-slate-600 italic mt-0.5 line-clamp-1 max-w-xl">
                      "{order.smsContent}"
                    </p>
                  )}
                </div>
              ...
              <button
                ...
                onClick={() => {
                  const res = StoreService.sendOrderConfirmationSMS(order);
                  setSmsSentSuccess(`অর্ডার #${order.orderNumber} এর জন্য কনফার্মেশন SMS পুনরায় প্রেরণ করা হয়েছে!`);
                  setTimeout(() => setSmsSentSuccess(null), 4500);
                  const refreshed = StoreService.getOrderById(order.id);
                  if (refreshed) {
                    setOrder(refreshed);
                ...
                className="px-3 py-1 bg-white border border-slate-200 hover:bg-slate-100 text-slate-800 font-bold rounded-lg text-[11px] shrink-0 cursor-pointer shadow-2xs self-end sm:self-auto"
              >
                {order.smsSent ? 'পুনরায় SMS পাঠান' : 'এখনই SMS পাঠান'}
              </button>
            </div>
