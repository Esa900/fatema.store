...
import { 
  ...
  Trash2, RefreshCcw, Tag, Send, FileText
} from 'lucide-react';
...
export const OrdersTab: React.FC<OrdersTabProps> = ({
  ...
  // SMS Dispatch toast feedback
  const [smsSentToast, setSmsSentToast] = useState<string | null>(null);

  const handleSendSMS = (order: Order) => {
    try {
      const sms = StoreService.sendOrderConfirmationSMS(order);
      setSmsSentToast(`অর্ডার #${order.orderNumber} এর জন্য গ্রাহক (${order.phone})-কে সফলভাবে SMS প্রেরণ করা হয়েছে!`);
      setTimeout(() => setSmsSentToast(null), 4500);
      if (onRefreshOrders) {
        onRefreshOrders();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Orders List Table */}
      {smsSentToast && (
        <div className="p-3.5 bg-emerald-50 border border-emerald-300 text-emerald-800 rounded-xl text-xs font-bold flex items-center justify-between shadow-xs animate-in fade-in">
          ...
          <button 
            ...
            className="text-emerald-700 hover:text-emerald-950 font-bold ml-3 cursor-pointer"
          >
            ✕
          </button>
        </div>
                    ...
                    <tr key={order.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-4 text-center">
                        <div className="flex items-center justify-center gap-1.5 flex-wrap">
                          ...
                          {/* 1-Click Print Invoice Memo */}
                          <button
                            type="button"
                            onClick={() => onPrintInvoice(order, 'invoice')}
                            title="১-ক্লিকে ইনভয়েস মেমো প্রিন্ট করুন"
                            className="p-1.5 bg-slate-100 hover:bg-emerald-100 hover:text-emerald-800 text-slate-700 rounded-lg transition-colors cursor-pointer border border-slate-200"
                          >
                            <Printer className="w-3.5 h-3.5" />
                          </button>

                          {/* 1-Click Courier Shipping Label Sticker Print */}
                          <button
                            type="button"
                          ...
                          {/* 1-Click Send / Resend SMS */}
                          <button
                            type="button"
                            onClick={() => handleSendSMS(order)}
                            title={order.smsSent ? `SMS পাঠানো হয়েছিল: ${order.smsSentAt ? new Date(order.smsSentAt).toLocaleTimeString() : 'হ্যাঁ'} (পুনরায় পাঠাতে ক্লিক করুন)` : "গ্রাহকের মোবাইলে কনফার্মেশন SMS পাঠান"}
      ...
      {detailsOrder && (
        <AdminOrderDetailsModal
          ...
          onPrintInvoice={(ord, mode) => onPrintInvoice(ord, mode)}
