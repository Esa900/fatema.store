...
export const StoreService = {
  ...
  sendOrderConfirmationSMS(order: Order): SMSLog {
    ...
    if (settings.smsEnabled === false) {
      ...
      const disabledLog: SMSLog = {
        ...
        message: 'SMS notifications currently disabled in settings',
    ...
    const template = settings.smsTemplate || `প্রিয় [NAME], ${storeName}-এ আপনার অর্ডার [ORDER_ID] সফলভাবে গ্রহণ করা হয়েছে! সর্বমোট: ৳[TOTAL]। দ্রুততম সময়ে ডেলিভারির ব্যবস্থা করা হচ্ছে। হেল্পলাইন: ${hotline}`;
    ...
    const smsLog: SMSLog = {
      ...
      sentAt: new Date().toISOString(),
      status: 'Delivered',
      provider: settings.smsProvider || 'Greenweb / BulkSMS BD Gateway'
    };
  ...
  sendCourierTrackingSMS(order: Order, courier: string, trackingId: string): SMSLog {
    const settings = this.getSettings();
    const hotline = settings.hotline || settings.phone || '01854774406';
    const storeName = settings.storeName || 'KHAN store';
    
    const trackingTemplate = settings.smsTrackingTemplate || 
      `প্রিয় [NAME], ${storeName} থেকে আপনার অর্ডার [ORDER_ID] কুরিয়ারে ([COURIER]) হ্যান্ডওভার করা হয়েছে। ট্র্যাকিং কোড: [TRACKING_ID]। দ্রুততম সময়ে ডেলিভারি নিশ্চিত হবে। হেল্পলাইন: ${hotline}`;

    const message = trackingTemplate
      .replace(/\[NAME\]/g, order.customerName)
      .replace(/\[ORDER_ID\]/g, order.orderNumber)
  ...
  sendCustomOrderSMS(orderId: string, customMessage: string): SMSLog | null {
    ...
    if (target) {
      target.smsSent = true;
      target.smsSentAt = smsLog.sentAt;
      target.smsContent = customMessage;
      setItem(STORAGE_KEYS.ORDERS, orders);
    }
  ...
  sendTestSMS(phone: string): SMSLog {
    const settings = this.getSettings();
    const hotline = settings.hotline || settings.phone || '01854774406';
    const storeName = settings.storeName || 'KHAN store';
    const message = `[TEST SMS] প্রিয় গ্রাহক, ${storeName}-এর স্বয়ংক্রিয় এসএমএস নোটিফিকেশন গেটওয়ে সক্রিয় রয়েছে। হেল্পলাইন: ${hotline}`;
    ...
    const smsLog: SMSLog = {
      ...
      recipientName: 'সম্মানিত টেস্ট গ্রাহক',
      message,
      sentAt: new Date().toISOString(),
      status: 'Delivered',
      provider: settings.smsProvider || 'Greenweb BD'
    };
  ...
  updateOrderCourier(orderId: string, courierService: string, courierTrackingId: string): void {
    ...
    if (order) {
      ...
      // Auto-dispatch tracking SMS to customer
      try {
        if (courierTrackingId) {
          const sms = this.sendCourierTrackingSMS(order, courierService, courierTrackingId);
          order.smsSent = true;
          order.smsSentAt = sms.sentAt;
          order.smsContent = sms.message;
        }
      } catch (err) {
        console.warn('Tracking SMS dispatch error:', err);
      }
